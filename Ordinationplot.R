...
df_site <- scores(rdamodel,display='cn')
...
ggplot(df_species,aes(x=RDA1, y=RDA2))+
  ...
  #Check df_environ in the console to manually know the coordinates
  ...
  geom_segment(data=df_environ[9,1:2,drop=FALSE], 
               ...
               ,size=0.8,arrow=arrow(length=unit(0.02,"npc")))+
  geom_segment(data=df_environ[10,1:2,drop=FALSE], 
               aes(x=0, xend=RDA1*scaling_factor,y=0, 
                   yend=RDA2*scaling_factor,color="pH")  
               ,size=0.8,arrow=arrow(length=unit(0.02,"npc")))+
  geom_segment(data=df_environ[11,1:2,drop=FALSE], 
               aes(x=0, xend=RDA1*scaling_factor,y=0, 
                   yend=RDA2*scaling_factor,color="Temperature")
               ,size=0.8,arrow=arrow(length=unit(0.02,"npc")))+  
  geom_segment(data=df_environ[12,1:2,drop=FALSE], 
               aes(x=0, xend=RDA1*scaling_factor,y=0, 
                   yend=RDA2*scaling_factor,color="DO")
               ,size=0.8,arrow=arrow(length=unit(0.02,"npc")))+
  #Sites    
  geom_point(data=df_site[1,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray24",size=1.5)+
  geom_text(data=df_site[1,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray25",
            label="Bridge",size=2,face="bold",nudge_y=-0.06)+
  geom_point(data=df_site[2,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray24",size=1.5)+
  geom_text(data=df_site[2,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray25",
            label="Delta",size=2,face="bold",nudge_y=-0.06)+
  geom_point(data=df_site[3,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray24",size=1.5)+
  geom_text(data=df_site[3,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray25",
            label="River.A",size=2,face="bold",nudge_y=-0.07)+
  geom_point(data=df_site[4,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray24",size=1.5)+
  geom_text(data=df_site[4,1:2,drop=FALSE],aes(x=RDA1,y=RDA2),color="gray25",
